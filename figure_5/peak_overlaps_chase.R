...
sample_prefix <- c("Dox0_chase", "Dox75_chase", "Dox500_chase")
...
all_peaks$peak_in_0_chase  <- F
all_peaks$peak_in_75_chase  <- F
all_peaks$peak_in_500_chase <- F
...
for(j in 1:length(sample_prefix)){
  ...
  if((j == 1) ){
    ...
    all_peaks$peak_in_0_chase[overlap_indices] <- T
    
  }
  if((j == 2 )){ 
    all_peaks$peak_in_75_chase[overlap_indices] <- T
  }
  if((j == 3)){
    all_peaks$peak_in_500_chase[overlap_indices] <- T
  }
...
for(h in 1:length(all_peaks)){
  ...
  if(all_peaks[h]$peak_in_0_chase){
    build_category_string <- c(build_category_string, "0c")
  }
  if(all_peaks[h]$peak_in_75_chase){
    build_category_string <- c(build_category_string, "75c")
  }
  if(all_peaks[h]$peak_in_500_chase){
    build_category_string <- c(build_category_string, "500c")
  }
...
saveRDS(all_peaks,"~/dosage_manuscript/rds/peak_categories_GR_103024_chase.rds")

save_GR <- as.data.frame(all_peaks)

write.table(save_GR, file="~/dosage_manuscript/rds/peak_categories_GR_103024_chase.tsv",sep="\t")
...
for(g in 1:length(categories)){
  ...
  write.table(subset_peaks, file=paste0("~/repliATAC/peak_categories/",categories[g],"_peaks_103024_chase.bed"),sep="\t", col.names=F, row.names=F,quote=F)
  
  
}

all_peaks <- read.table("~/dosage_manuscript/csv/peak_categories_GR_103024_chase.tsv",sep="\t")
...
#unique IDR peaks
select_categories <- c("0c", "75c","500c" , "0c_75c_500c")
...
#for source data
write.csv(dbt.df, "~/dosage_manuscript/csv/peaks_chase.csv")
